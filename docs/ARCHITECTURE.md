...
classDiagram
    ...
    class PreviewPlayer {
        <<RT audition renderer on the metronome's cue-fold lane (post-graph, never recorded/metered; spec P1 §12 preview-before-loading). Claims offered PreviewClips at block boundaries; plays one-shots 1:1 (cache conformed the rate at load; tempo-synced loop preview joins the time-stretch milestone). CLICK DISCIPLINE (cycle-3 hardened): 5ms fade-in, a 5ms tail fade INTO the buffer end (truncated files close clean), and a replaced preview fades OUT while the new fades in - the captured retire gain FOLDS IN the tail factor so a clip replaced inside its own tail never jumps back up; the retiring artifact's ack is DEFERRED until its fade completes (builder must not free mid-fade); a third claim inside one fade hard-drops the near-silent oldest. LIFETIME: a finished/stop/failed clip is released (acked + handle unpinned) as soon as no OLDER artifact is still fading - never acking a newer epoch past a live fade, and never pinning a finished audition's file for the session. prepare() [non-RT, streams closed] releases held claims AND drains the unclaimed offer (a stale-rate clip must never play off-speed after reopen) - the one legal non-RT ack window>>
    ...
    class ProjectAction {
        <<sealed interface; v1.2 media additions: AssignSampleToDevice (trackId, deviceId, slot, fileId, path, name - a document edit, undoable; fileId 0 clears), LoadSampleIntoNewSampler (the Simpler-on-drop gesture: appends a SAMPLER holding the sample + baked root note in ONE action/one undo step - cycle-3: LOAD must create when no sampler exists, never no-op), and transient PreviewSample/StopPreview (state-untouched, undo-excluded like ToggleMetronome - EngineSync forwards them because dispatch notifies unconditionally)>>
    }
    ...
    class SoundBrowserStateHolder {
        <<preset rows + the FactoryPack samples merged reactively into SAMPLES_LOOPS (BrowserItem.sample marks playable media). togglePreview dispatches PreviewSample/StopPreview (toggle semantics; previewingItemId is holder-local UI state); assignToSampler loads slot 0 of the selected track's first SAMPLER (fallback: first sampler anywhere) + sample.root param, and with NO sampler in the project dispatches LoadSampleIntoNewSampler onto the best instrument-capable lane (selected, else first MIDI/AUDIO/DRUM - cycle-3: the default project ships samplerless, LOAD must create); the only failure (zero tracks) surfaces as lastLoadFailedItemId ("NO TRACK" row state), never silent; drum-pad drag assignment joins the drum lab pass>>
    ...
    %% ==========================================
    class FactoryPack {
        <<object (data.media): the generated factory starter library (spec P1 §12). Eight one-shot WAVs SYNTHESIZED deterministically on first run into filesDir/factory/v1 (fixed-seed LCG noise; 44.1k mono 16-bit; write + fsync + rename, and the completeness gate demands length > 44 so a crash-window husk never passes - cycle-3 hardened) - no binaries in repo or APK. Recipes: 808 kick (exp pitch drop + tanh drive), two-component snare, four-burst room clap, 808 metal-cluster hats (closed/open), Karplus-Strong pluck C3, filtered saw+sub bass C2, detuned Am stab; every sound peak-normalized to -1 dBFS with a baked 15 ms end fade (SimpleSampler's one-shot end is a hard cut - files must close themselves clean). fileId = fnv1a64 of the LIBRARY-RELATIVE path (stable across devices while absolute paths differ); rootNote metadata keys melodic assignment>>
    ...
    class EngineSync {
        <<the change-classification seam (dual-model), complete for M1: transport intents -> messages, param moves -> Param/Move, structure edits -> ModelDelta bundles - all stamped with the store's real editSeq. Cascading removes derive from the PRE-change state; shared ClipContent removed only when unreferenced in post-state; canonical content id of a linked arr/session pair = lexicographic MIN of the clip ids (forward-compatible with explicit ClipContent + copy-on-unlink at the session milestone). Drum steps flatten to NoteRecords via DrumPadType.midiPitch with stable fnv32 step ids. Session intents (M4) -> seam-2 Session ops: a slot press launches the clip at that slot or, empty, sends the stop (mirroring the reducer's isPlaying marks); TriggerScene fans one launch/stop per track in ONE flush so every lane shares the boundary; the store's isPlaying/isOverriddenBySession flags are the optimistic UI (engine slot readback deferred). Media (v1.2): AssignSampleToDevice -> one SampleRef frame keyed by the device uid; LoadSampleIntoNewSampler -> chain resend (device row w/ baked sample.root + ref, one bundle); PreviewSample/StopPreview -> Preview frames (transient, ride the same ordered delta path); refs are SET-MEMBERSHIP, so full pushes AND chain resends reset-then-readd per device (byteLen-0 remove-all first - cycle-3 fix: an undo that empties a slot now reaches the engine); device/track removals send explicit SampleRef removes unconditionally (the model never cascades). NULL store action (undo/redo) -> full model push WITH a PRE->POST membership diff (encodeMembershipRemovals: explicit removes for tracks/devices/clips/scenes the undo removed, same bundle as the upserts - closes the ghost-sampler/ghost-track undo hole one step deep); every RUNNING transition -> plain full push + param resend; divergence older than one step (missed bundles) waits on the general engine-side mark-and-sweep, TRACKED for the persistence milestone>>
        +attach()
        +detach()
